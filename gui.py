...
plt.subplots_adjust(bottom=0.35) # Reserva espaço inferior para os controles deslizantes

# Realiza a primeira simulação para preencher o gráfico ao abrir a janela
tvec, P2_washing_out = simulate_ramsey_fringe(T0_init*1e-6, T_init, t_init, B_fixed)
...
info_text = fig.text(0.5, 0.28, '', fontsize=11, ha='center', va='center', 
                     bbox=dict(facecolor='#f0f0f0', alpha=0.8, edgecolor='gray'))

# Definição dos eixos dos sliders: [esquerda, base, largura, altura]
ax_T0 = plt.axes([0.2, 0.18, 0.65, 0.03])
ax_T  = plt.axes([0.2, 0.11, 0.65, 0.03])
ax_t  = plt.axes([0.2, 0.04, 0.65, 0.03])
...
def update(val):
    ...
    Omega_mw_0 = np.pi / (2 * t_val)
    
    # Atualiza o display das variáveis no gráfico
    info_text.set_text(f'Vel. de Lançamento: $V_z$ = {v_launch:.2f} m/s  |  Freq. de Rabi: $\\Omega_{{0}}$ = {Omega_mw_0:.2f} rad/s')

    # Refaz o cálculo de expansão de Monte Carlo com os novos parâmetros
    new_tvec, new_P2 = simulate_ramsey_fringe(T0_val, T_val, t_val, B_fixed)
