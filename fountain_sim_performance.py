...
def simulate_ramsey_fringe(Temp_0, T, t, B=3e-9, theta1=np.pi / 2,
                           ...
                           sigma=0.2):
    """
    Simulação otimizada das franjas de Ramsey com pulso não-instantâneo.

    Parâmetros
    ----------
    Temp_0, T, t, B, theta1, theta2 : idênticos a ``fountain_sim_alt``.
    N_r  : número de pontos de Gauss-Legendre (integral radial, [0, Rc]).
    ...
    profile1, profile2 : 'sine', 'blackman' ou 'square'.
    tvec : (opcional) array de dessintonia normalizada (Delta*tau) onde avaliar.
           Se None, gera a varredura padrão em [-20, 20].
    ...
    # ---- Quadratura radial (Gauss-Legendre em [0, Rc]) -------------------
    x_r, w_r = np.polynomial.legendre.leggauss(N_r)
    r_grid = 0.5 * Rc * (1.0 + x_r)
    weights_r = w_r * (0.5 * Rc) * r_grid * np.exp(-(r_grid**2) / (2.0 * sigma_r**2))
    s_r = weights_r.sum()
    if s_r <= 0.0:
    ...
    t_local = t * (1.0 - V_flat / v_launch)                 # (N_total,)
    T_local = T * (1.0 + V_flat / v_launch)
